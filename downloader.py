...
class Downloader():
    ...
    def download(self, link_video, isAudio, **kwargs):
        ...
        if (isAudio):
            yt.streams.filter(mime_type='audio/mp4').first().download(filename=f'audio_{title}', output_path=full_path)
        else:
            quality = kwargs.get('quality', None)
            audio_file = f'audio_{title}'
            video_file = f'video_{title}'
            yt.streams.filter(mime_type='audio/mp4').first().download(filename=audio_file, output_path=full_path)
            yt.streams.filter(mime_type='video/mp4', res=quality).first().download(filename=video_file, output_path=full_path)
